...
Page({
  ...
  //获取活动详情
  queryBannerConfigDetail(addCart) {
    let t = this;
    if (!addCart) {
      wx.showLoading({title: "加载中..."});
    }
    service.queryBannerConfigDetail({
      ...
      this.data.firstList.map((item) => {
        ...
        goods.map((goodsObj) => {
          ...
          if (carts) {
            carts.map((cart) => {
              if (
                cart.goodsCode == goodsObj.skuCode 
              ) {
                goodsObj.carCode = cart.cartCode   //商品减少为0(相当于删除)需要cartcode
  ...
  addNewCart(goods) {
    wx.showLoading({title: "加载中..."});
    const userId = wx.getStorageSync('userId');
    const {skuCode, cartNum, activityId} = goods;
    service.addgoodnum({
      branchesId: this.data.branchesId,
  ...
  delNewCart(e){
    ...
    const userId = wx.getStorageSync('userId');
    let { skuCode, cartNum, activityId, carCode} = dooditem
      if (cartNum>1){
        service.addgoodnum({
          branchesId: this.data.branchesId,
          activityId: activityId,
